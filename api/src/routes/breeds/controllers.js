...
async function getInfoApi() {
  try {
    ...
    const dataApi = await Promise.all(
      url.data.map(async e => {
        ...
        return {
          ...
          weight: {
            min: !isNaN(wmin) ? wmin : 0,
            max: !isNaN(wmax) ? wmax : 0,
          },
    ...
    if (error?.response?.status === 429) {
      ...
      console.error('Error: -->getInfoApi', error?.response?.status)
      throw error
    }
  }
}
...
async function getBreeds(req, res) {
  ...
  try {
    ...
    res?.status(400)?.send({ data: error + '-->getBreeds' })
  }
}
...
async function breedForId(req, res) {
  ...
  try {
    ...
    res?.status(404)?.json({ data: error + '-->breedForId' })
  }
}
